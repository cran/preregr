...
In addition to being able to specify [(pre)registration forms from within R](https://r-packages.gitlab.io/preregr/articles/creating_prereg_form.html), you can also specify the form using a spreadsheet. You can use an Excel file (`.xlsx`) or a Google Sheet.
...
The `metadata` worksheet contains two columns (in whichever order): `heading` and `description`. Every row holds a different bit of metadata. The field name is specified in the `field` column and the metadata itself is specified in the `content` column in the corresponding row. Always specify content for at least the following fields: specify the form's title as `title`; specify the form's author(s) as `author`, specify the date as `date`, and specify the version as `version`.
...
The `instructions` worksheet contains two columns (in whichever order): `heading` and `description`. Every row holds one specific set of instructions in the `descriptions` columns that will be included under the heading in the `heading` column.
...
To import a form specification in a spreadsheet, use [preregr::read_spreadsheet()]. For example:
